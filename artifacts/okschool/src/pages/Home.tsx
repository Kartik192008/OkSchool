...
export function Home() {
  ...
  return (
    <div>
      ...
      {/* Tab navigation */}
      <div className="border-b bg-background sticky top-[113px] z-40">
                ...
                <div className="group bg-card border border-card-border rounded-lg overflow-hidden hover:shadow-md transition-shadow cursor-pointer h-full">
                  <div className="relative bg-muted aspect-[3/4] flex items-center justify-center">
                    {doc.thumbnailUrl ? (
                      <img 
                        ...
                        onError={(e) => {
                          console.error('Image load error:', doc.thumbnailUrl, e);
                          (e.target as HTMLImageElement).style.display = 'none';
                        }}
                        onLoad={() => console.log('Image loaded successfully:', doc.thumbnailUrl)}
                      />
                    ) : (
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                      {doc.isFree ? (
                        <span className="flex items-center gap-1"><Download className="h-3 w-3" /> Free PDF</span>
                      ) : (
                        <span className="flex items-center gap-1"><Lock className="h-3 w-3" /> Paid Word</span>
                      )}
                    </div>
                  </div>
                </div>
